...
额外抽跑旧 `app.spec.ts` 的 `today task opens the focused study flow and enforces score 59 revision`，两种 viewport 都未通过：补齐其既有 `/me/entitlements` mock 后，停在第 257 行的精确 `预计 8 分钟` 定位；当前 DOM 文本为 `预计8 分钟`。本次没有扩修无关的旧学习断言，也没有运行全部 E2E 或真实 Google 登录，不能报告全套端到端通过。
