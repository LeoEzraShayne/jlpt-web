...
- 登录和 `AuthGate` 共用一个 18+ 确认组件，复用当前浏览器已明确确认的状态；登录返回与 Android 绑定不各加一套确认。checkbox 不预选，必须点击确认后才挂载其下的登录或受保护内容；旧登录会话也经过此 UI。
- 确认保存于带版本键的 `localStorage`，兼容已有 `sessionStorage` 确认；退出登录清除两处，并向其他窗口同步撤销。浏览器存储不可用时仅保留当前页面内存状态。只有实际存在的显式确认可迁移；已经丢失的旧标签页状态不能按 Google 登录或 VIP 推断恢复，需本人重新确认一次。没有收集出生日期、身份证件，也没有声称 Google 已验证年龄。
