...
const description =
  "JLPT Sentence Lab是提供中英文体验的 JLPT N1～N4 日语语法学习工具，通过日语造句、AI 语法批改、个性化计划和间隔复习帮助学习者把语法用于实际表达。";
...
const features = [
  {
    icon: Library,
    title: "N1～N4 语法库",
    text: "按 JLPT 级别查看语法点、中英文解释、接续方式和日语例句。",
  },
  {
    icon: PenLine,
    title: "以造句练习输出",
...
const faq = [
  {
    question: "JLPT Sentence Lab是做什么的？",
    answer:
  ...
  },
  {
    question: "适合哪些学习者？",
    answer:
      "适合正在准备 JLPT N1、N2、N3 或 N4，希望从“看得懂”进阶到“写得出、用得对”的日语学习者。",
  },
  {
  ...
  },
  {
    ...
    answer:
      "官网介绍无需登录。使用 Google 登录可保存学习进度；免费版每天提供 5 个语法与词汇任务，每任务 3 次成功批改。会员为一次购买、不自动续费，具体价格与开放状态见会员页面。",
  },
];
  ...
  "@graph": [
    {
      "@type": "WebSite",
      "@id": `${siteUrl}/#website`,
      name: "JLPT Sentence Lab",
      url: `${siteUrl}/`,
      description,
      inLanguage: ["zh-CN", "en", "ja-JP"],
    },
    {
      ...
      <main>
          ...
          <div className="mx-auto grid max-w-[76rem] items-center gap-12 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[1.1fr_.9fr] lg:px-8 lg:py-28">
            <div>
              <p className="inline-flex items-center gap-2 rounded-full border bg-card px-4 py-2 text-sm font-medium">
                ...
                {t("JLPT N1～N4 · 中英文解释 · Web 应用")}</p>
              <h1 className="mt-6 max-w-4xl text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
                {t("通过造句，真正掌握")}<span className="block text-secondary-foreground">{t("日语语法")}</span>
              </h1>
              <p className="mt-6 max-w-3xl text-lg leading-8 text-foreground/75 sm:text-xl">
                {t("JLPT Sentence Lab帮助中文使用者学习 JLPT N1～N4 语法。阅读解释、主动造句、获得 AI 批改，再按记忆状态复习， 把“看得懂”逐步变成“用得出”。")}</p>
              <div className="mt-8 flex flex-wrap gap-3">
                <Button asChild size="lg">
                  <Link href="/login">
                    {t("免费开始学习")}<ArrowRight className="size-5" />
                  </Link>
                </Button>
                <Button asChild size="lg" variant="outline">
                  <Link href="#how-it-works">{t("了解学习方式")}</Link>
                </Button>
              </div>
              <p className="mt-4 text-sm text-muted-foreground">
                {t("官网介绍无需登录；保存学习进度需要 Google 账号。")}</p>
            </div>
            <div className="rounded-[2rem] border bg-card p-6 shadow-2xl shadow-primary/10 sm:p-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">JLPT Sentence Lab</p>
                  <h2 className="text-xl font-bold">{t("一次完整的语法练习")}</h2>
                </div>
              </div>
                ...
                {[
                  ...
                  <li key={number} className="flex gap-4 rounded-2xl border bg-background p-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm leading-6 text-muted-foreground">{t(text)}</p>
                    </div>
                  </li>
                ))}
              </ol>
        ...
        <section id="features" className="scroll-mt-20 px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          <div className="mx-auto max-w-[76rem]">
            <div className="max-w-3xl">
              <p className="font-semibold text-secondary-foreground">{t("核心功能")}</p>
              <h2 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">
                {t("围绕“理解、输出、反馈、复习”设计")}</h2>
              <p className="mt-4 text-lg leading-8 text-muted-foreground">
                {t("不是批量生成的语法词条页，而是一套需要登录后使用的学习流程。")}</p>
            </div>
            <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {features.map(({ icon: Icon, title, text }) => (
                <article key={title} className="rounded-2xl border bg-card p-6">
                  ...
                  <p className="mt-2 leading-7 text-muted-foreground">{t(text)}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section id="how-it-works" className="scroll-mt-20 border-y bg-card px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          <div className="mx-auto grid max-w-[90rem] gap-10 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,.65fr)] xl:items-center">
            <div>
              <p className="font-semibold text-secondary-foreground">{t("适用场景")}</p>
              <h2 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl xl:whitespace-nowrap">
                {t("适合想把 JLPT 语法用于表达的学习者")}</h2>
              <p className="mt-5 text-lg leading-8 text-muted-foreground">
                <span className="block xl:whitespace-nowrap">
                  {t("如果你正在准备 N1～N4，能读懂中文语法解释，却经常在写作或会话中想不起怎么用，")}</span>
                <span className="block">{t("JLPT Sentence Lab可以把每天的学习拆成可执行的小任务。")}</span>
              </p>
            </div>
            <ul className="grid gap-4 sm:grid-cols-2">
              {[
                ...
                <li key={item} className="flex items-start gap-3 rounded-2xl border bg-background p-4">
                  ...
                  <span className="leading-7">{t(item)}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <section id="faq" className="scroll-mt-20 px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          <div className="mx-auto max-w-4xl">
            <div className="text-center">
              <p className="font-semibold text-secondary-foreground">{t("常见问题")}</p>
              <h2 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">{t("关于JLPT Sentence Lab")}</h2>
            </div>
            <div className="mt-10 space-y-4">
              {faq.map((item) => (
                <article key={item.question} className="rounded-2xl border bg-card p-6">
                  ...
                  <p className="mt-3 pl-8 leading-7 text-muted-foreground">{t(item.answer)}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="px-4 pb-16 sm:px-6 sm:pb-20 lg:px-8">
          <div className="mx-auto flex max-w-[76rem] flex-col items-start justify-between gap-6 rounded-[2rem] bg-primary p-8 text-primary-foreground sm:p-10 md:flex-row md:items-center">
            <div>
              <p className="flex items-center gap-2 font-semibold"><BookOpenText className="size-5" /> JLPT Sentence Lab</p>
              <h2 className="mt-3 text-2xl font-bold sm:text-3xl">{t("从下一个语法点开始主动输出")}</h2>
            </div>
            <Button asChild size="lg" variant="secondary">
              <Link href="/login">{t("进入学习空间")}<ArrowRight className="size-5" /></Link>
            </Button>
          </div>
        </section>
      </main>
