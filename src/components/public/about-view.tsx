    ...
    <PublicPage>
      ...
      <main className="mx-auto max-w-4xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
        <nav className="text-sm text-muted-foreground" aria-label={t("面包屑")}>
          <Link href="/" className="hover:text-foreground">{t("首页")}</Link> {t("/ 关于")}</nav>
        <h1 className="mt-6 text-4xl font-bold tracking-tight sm:text-5xl">{t("关于JLPT Sentence Lab")}</h1>
        <p className="mt-6 text-lg leading-8 text-foreground/75">
          {t("JLPT Sentence Lab是一款提供中英文体验的 JLPT N1～N4 日语语法学习 Web 应用。 它把语法解释、主动造句、AI 批改、学习计划和间隔复习组合成一套持续练习流程。")}</p>

        <section className="mt-12 grid gap-5 sm:grid-cols-3">
          {[
            { icon: Languages, title: "学习范围", text: "JLPT N1、N2、N3、N4 日语语法" },
            { icon: UsersRound, title: "目标用户", text: "希望理解语法并主动用日语表达的学习者" },
        ...
        <section className="mt-12 rounded-2xl border bg-card p-6 sm:p-8">
          <h2 className="text-2xl font-bold">{t("站点身份")}</h2>
          <dl className="mt-6 grid gap-5 sm:grid-cols-[10rem_1fr]">
            <dt className="font-medium">{t("项目名称")}</dt>
            <dd className="text-muted-foreground">JLPT Sentence Lab</dd>
            <dt className="font-medium">{t("官方网站")}</dt>
            <dd><Link href="/" className="text-secondary-foreground hover:underline">https://jlpt.meritledger.org/</Link></dd>
            <dt className="font-medium">{t("项目类型")}</dt>
            <dd className="text-muted-foreground">{t("日语语法学习 Web 应用")}</dd>
            <dt className="font-medium">{t("使用语言")}</dt>
            <dd className="text-muted-foreground">{t("中英文界面与讲解，日语原文保持日语")}</dd>
          </dl>
        </section>

        <section className="mt-12">
          <h2 className="text-2xl font-bold">{t("联系与反馈")}</h2>
          <p className="mt-4 leading-7 text-muted-foreground">
            {t("功能问题、内容纠错或项目反馈可通过公开 GitHub 仓库提交。 请不要在公开 issue 中填写邮箱、账号信息、学习记录或其他个人敏感信息。")}</p>
          <a
          ...
          >
            <Code2 className="size-5" /> {t("前往 GitHub Issues")}<ExternalLink className="size-4" />
          </a>
        </section>
      </main>
    </PublicPage>
