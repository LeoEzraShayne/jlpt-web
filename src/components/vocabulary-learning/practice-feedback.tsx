...
export function PracticeFeedback({ practice: rawPractice, showSchedule = true }: { practice: VocabularyPractice; showSchedule?: boolean }) {
  useLocale();
  const practice = vocabularySnapshot(rawPractice);
  ...
  return <Card className="min-w-0 warm-shadow">
    ...
    <CardContent className="min-w-0 space-y-4 break-words">
      <div><h3 className="text-sm font-semibold">{t("你的句子")}</h3><p lang="ja" className="mt-2 whitespace-pre-wrap leading-8">{practice.answer || t("未记录")}</p></div>
      {result ? <>
      ...
      {showSchedule && <p className="text-sm font-medium">{result?.outcome === "UNVERIFIED" ? t("保留原复习安排，明天可再次验证；今天也可以从清单主动练习。") : <>{t("下次复习：")}{reviewDate(practice.nextReviewAt)}</>}</p>}
