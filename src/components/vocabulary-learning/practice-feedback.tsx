  ...
  return <Card className="min-w-0 warm-shadow">
    ...
    <CardContent className="min-w-0 space-y-4 break-words">
      ...
      {result ? <>
        ...
        {!!result.corrections.length && <div><h3 className="text-sm font-semibold">逐项修改</h3><ul className="mt-2 space-y-3 text-sm">{result.corrections.map((item, index) => <li key={index} className="rounded-lg border p-3"><p lang="ja">{item.text} → {item.replacement}</p><p className="mt-1 text-muted-foreground">{item.reason}</p></li>)}</ul></div>}
      ...
      <p className="text-sm font-medium">下次复习：{reviewDate(practice.nextReviewAt)}</p>
