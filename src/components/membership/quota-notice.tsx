...
export function QuotaSummary() {
  ...
  if (!data) return null;
  return <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border p-3 text-sm"><span>{t(data.quota.enforcementEnabled === false ? "当前尚未启用任务额度限制" : data.isMember ? "会员：正常学习不限任务与批改次数" : `今日免费任务剩余 ${data.quota.remaining}/${data.quota.dailyLimit}`)}</span><Button asChild size="sm" className="shrink-0 rounded-full max-lg:min-h-11"><Link href="/membership">{t("会员与额度")}</Link></Button></div>;
}
