...
export function PaymentReturn() {
  ...
  return <div className="mx-auto flex max-w-2xl flex-col gap-5"><Card><CardHeader><CardTitle>{t(paid ? "付款已确认" : "付款状态")}</CardTitle><CardDescription>{t("权益仅在服务端核实付款后开通。")}</CardDescription></CardHeader><CardContent className="flex flex-col gap-4" aria-live="polite">{!id ? <p>{t("缺少订单编号，请从订单记录查看。")}</p> : order.error ? <p role="alert">{t("暂时无法确认订单，请刷新或稍后从订单记录查看。")}</p> : <><p>{order.data ? orderStatus(order.data.status, t) : t("正在确认付款…")}</p>{paid ? <p>{t(membership.data?.isMember ? "会员已生效，可以继续学习。" : "付款已确认，当前会员状态请在会员页面查看。")}</p> : slow && <p>{t("确认比平时稍久。请勿重复付款，可稍后查看此订单。")}</p>}<p className="break-all text-xs text-muted-foreground">{id}</p></>}<Button variant="outline" onClick={() => { void order.mutate(); void membership.mutate(); }}>{t("刷新状态")}</Button></CardContent></Card><div className="flex flex-wrap gap-4 text-sm"><Link href="/today" className="underline">{t("继续学习")}</Link><Link href="/membership/orders" className="underline">{t("订单记录")}</Link><Link href="/membership" className="underline">{t("返回会员")}</Link></div></div>;
}
